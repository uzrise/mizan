...
function normalizeSlugValue(rawSlug) {
  if (!rawSlug) return '';
  const slugValue = Array.isArray(rawSlug) ? rawSlug[0] : rawSlug;
  const asString = typeof slugValue === 'string' ? slugValue : String(slugValue);
  let decoded = asString;
  try {
    decoded = decodeURIComponent(asString);
  } catch (error) {
    decoded = asString;
  }
  return decoded.trim();
...
function buildSlugCandidates(rawSlug) {
  const normalized = normalizeSlugValue(rawSlug);
  ...
  const slugified = lower
    ...
    .replace(/-+/g, '-')
    .replace(/^-|-$/g, '');
  if (slugified && !candidates.includes(slugified)) {
    candidates.push(slugified);
  }
  return candidates;
}

function findProjectInConstants(slugCandidates) {
  for (const candidate of slugCandidates) {
    const project = getProjectBySlugFromConstants(candidate);
    if (project) return project;
  }
  return null;
}

async function fetchProjectBySlugValue(slug) {
...
export async function getProjectBySlug(slug) {
  ...
  try {
    for (const candidate of slugCandidates) {
      const project = await fetchProjectBySlugValue(candidate);
      if (project) {
        return project;
      }
    }
    ...
    const fallbackProject = findProjectInConstants(slugCandidates);
    if (fallbackProject) {
